...
| Stellar  | `getVersionInfo` [[27]](#ref-27)                   |
| NEAR     | `status` [[13]](#ref-13)                           |
| Sui      | `LedgerService.GetServiceInfo` [[15]](#ref-15)     |
| Logos L1 | `GET /version` [[19]](#ref-19)                     |
| LEZ      | `checkHealth`, no version reported [[21]](#ref-21) |
...
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| Software version        | Cosmos `version`, `git_commit`, `build_tags`, `go_version` [[10]](#ref-10); Logos L1 a bare string of package version and commit [[19]](#ref-19) |
| Framework version       | Cosmos `cosmos_sdk_version` [[10]](#ref-10)                                                                                                      |
| Per-dependency versions | Cosmos `build_deps`, a repeated module path, version, and checksum [[10]](#ref-10)                                                               |
| Service identity        | Cosmos `name`, `app_name` [[10]](#ref-10)                                                                                                        |

The spread is the finding. Cosmos returns a dependency manifest, so a client can
detect capability per module rather than per node build [[10]](#ref-10). Logos
L1 returns a bare JSON string, which leaves no field structure to carry a
network identifier, feature flags, or retention bounds even if it wanted to
[[19]](#ref-19).
...
`cosmos_sdk_version` [[10]](#ref-10). That makes version-conditional capability
negotiation a property of the contract rather than of documentation.
...
- **Logos L1**: `GET /version` returns a bare JSON string, so there is no field
  ...
  handler discards all four [[119]](#ref-119).
- **LEZ**: No FFI reports a node version or build identity. The `lez_core`
  module returns its own module version [[120]](#ref-120), and the sequencer's
...
| Stellar  | `getHealth` [[27]](#ref-27)                                                           |
| NEAR     | `health` [[13]](#ref-13)                                                              |
| Sui      | [NOT FOUND] [[15]](#ref-15)                                                           |
| Logos L1 | `GET /cryptarchia/info` [[19]](#ref-19)[[118]](#ref-118)                              |
| LEZ      | Indexer `getStatus`; sequencer `checkHealth` is a stub [[22]](#ref-22)[[21]](#ref-21) |
...
| What the caller learns  | Where it appears                                                                                                                                                             |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Is the node caught up   | Cosmos `syncing` [[10]](#ref-10); Bitcoin `initialblockdownload` [[105]](#ref-105); Logos L1 `state` [[19]](#ref-19); LEZ indexer `state` [[115]](#ref-115)                  |
| How far caught up       | Bitcoin `verificationprogress`, normalised 0 to 1 [[105]](#ref-105)                                                                                                          |
| Current tip             | Cosmos `latest_block_height` [[10]](#ref-10); Bitcoin `blocks` [[105]](#ref-105); Logos L1 `tip`, `height`, `slot` [[19]](#ref-19); LEZ `indexed_block_id` [[115]](#ref-115) |
| Headers ahead of blocks | Bitcoin `headers` [[105]](#ref-105)                                                                                                                                          |
| Retention floor         | Cosmos `earliest_block_height` [[10]](#ref-10); Bitcoin `pruneheight` [[105]](#ref-105)                                                                                      |
| Retention as a range    | XRPL `complete_ledgers`, possibly disjoint [[101]](#ref-101)                                                                                                                 |
| Retention policy        | Bitcoin `pruned`, `automatic_pruning`, `prune_target_size` [[105]](#ref-105)                                                                                                 |
| Finality reached        | XRPL `validated_ledger` present, else `closed_ledger` [[101]](#ref-101)                                                                                                      |
| Cannot safely serve     | XRPL `amendment_blocked` [[101]](#ref-101)                                                                                                                                   |
| Why it stalled          | LEZ `stall_reason`, `last_error` [[115]](#ref-115)                                                                                                                           |
| Which sync phase        | Logos L1 `phase`: awaiting genesis, initial block download, prolonged bootstrap, or following [[118]](#ref-118)                                                              |
| Finality anchor         | Logos L1 `lib`, `lib_slot`, the last irreversible block [[19]](#ref-19)                                                                                                      |
...
is, and whether it is automatic [[105]](#ref-105). XRPL expresses retention as a
range expression rather than a floor, documented as possibly "a disjoint
sequence such as `24900901-24900984,24901116-24901158`", and signals finality by
which field is present rather than by a status enum [[101]](#ref-101).
...
diagnostics" [[115]](#ref-115), so a client can tell how far behind an
indexer-backed read is, and why. That disclosure is indexer-only: the sequencer,
which is the submission path, reports nothing, its `checkHealth` returning an
unconditional empty success [[119]](#ref-119).

Logos L1 reports sync at two granularities on one call. The consensus engine
state is binary, `Bootstrapping` or `Online` [[19]](#ref-19), while the service
phase is a four-stage lifecycle documented in the source: `AwaitingGenesisTime`
...
Period is running"), and `Following` ("Following the chain in real time")
[[118]](#ref-118). Distinguishing "not yet started" from "downloading" from
"following" is finer-grained than the boolean most surveyed chains return, and
the first phase is the only case in the survey where a node documents that it
will serve reads but not writes.

One caveat on that endpoint. The OpenAPI annotation declares the response body
as `CryptarchiaInfo` [[19]](#ref-19), but the project's own HTTP client
deserialises it as `ChainServiceInfo`, which wraps `CryptarchiaInfo` alongside
the `phase` field [[118]](#ref-118). The published contract and the first-party
client disagree about whether `phase` is part of the response.

**Potential gaps.**

- **Logos L1**: Finer-grained than most of the survey, reporting consensus state
  plus a documented four-stage phase on one call [[118]](#ref-118); no retention
  floor is reported, so a client cannot tell which history the node still holds.
- **LEZ**: The indexer discloses ingestion state, tip, and stall diagnostics
  [[115]](#ref-115), but the sequencer, which is the submission path, reports
  nothing: its `checkHealth` returns an unconditional empty success
  [[119]](#ref-119).
...
| Stellar  | `getNetwork` [[27]](#ref-27)                       |
| NEAR     | `genesis_config` [[13]](#ref-13)                   |
| Sui      | `sui_getChainIdentifier` [[17]](#ref-17)           |
| Logos L1 | Inscribed at genesis, not served [[119]](#ref-119) |
| LEZ      | `getChannelId` [[21]](#ref-21)                     |
...
impossible to determine the current actual blockchain using the RPC"
[[94]](#ref-94). The concrete failure is silent misconnection across a fork: "An
ETH/ETC client can accidentally connect to an ETC/ETH RPC endpoint without
knowing it unless it tries to sign a transaction or it fetches a transaction
that is known to have signed with a chain ID" [[94]](#ref-94). The chain
identifier and the replay protection are the same value; EIP-155 introduced it
into signing so that a transaction valid on one chain is invalid on its fork
[[86]](#ref-86).
...
- **Logos L1**: A validated chain identifier is inscribed at genesis and read at
  ledger initialisation [[119]](#ref-119), but no route returns it, so a client
  cannot detect that it has been pointed at the wrong network.
- **LEZ**: Neither FFI exposes a zone identifier. `getChannelId` exists on the
  ...
  no equivalent [[120]](#ref-120), so an application cannot confirm which zone
  it reads from. The nearest reachable value is `get_sequencer_addr`, a
  configured address rather than a chain identity [[120]](#ref-120).
...
| Stellar  | [NOT FOUND] as a method; OpenRPC document published [[12]](#ref-12)                              |
| NEAR     | [NOT FOUND] as a method; OpenAPI document published [[14]](#ref-14)                              |
| Sui      | [NOT FOUND] as a method; OpenRPC document published [[17]](#ref-17)                              |
| Logos L1 | `GET /api-docs/openapi.json`, Swagger UI at `/swagger-ui` [[20]](#ref-20)                        |
| LEZ      | `getSchema` on the indexer [[22]](#ref-22)                                                       |
...
necessary to serialize/deserialize its binary data" [[102]](#ref-102). The
schema is not static: the response was itself extended in xrpld 3.2.0 to add
...
- **Logos L1**: The served document omits five wired routes including both
  ...
  and registers three component schemas [[119]](#ref-119), so it indexes routes
  rather than describing payloads.
- **LEZ**: No FFI serves a machine-readable description. `getSchema` exists on
  the indexer RPC without an FFI export [[120]](#ref-120), and the module
  contract is generated from the C++ header by a line-oriented parser, so no
  artefact describes the reachable surface.
...
| Stellar  | `getLedgers` [[27]](#ref-27)                                                           |
| NEAR     | `block`, `chunk` [[13]](#ref-13)                                                       |
| Sui      | `sui_getCheckpoint` [[17]](#ref-17)                                                    |
| Logos L1 | `GET /cryptarchia/blocks/:id` [[19]](#ref-19)                                          |
| LEZ      | `getBlock`, indexer `getBlockById` and `getBlockByHash` [[21]](#ref-21)[[22]](#ref-22) |
...
| Stellar  | `getLatestLedger` [[27]](#ref-27)                                                  |
| NEAR     | `block` for the final block [[13]](#ref-13)                                        |
| Sui      | `sui_getLatestCheckpointSequenceNumber` [[17]](#ref-17)                            |
| Logos L1 | `GET /cryptarchia/info` [[19]](#ref-19)                                            |
| LEZ      | `getLastBlockId`, indexer `getLastFinalizedBlockId` [[21]](#ref-21)[[22]](#ref-22) |
...
- **LEZ**: `getLastBlockId` and `getLastFinalizedBlockId` return an identifier
  ...
  costs a second call [[21]](#ref-21)[[22]](#ref-22).
...
- **Logos L1**: No general subject query exists, and there is no lookup by note
  ...
  which returns 404 for any key the node does not hold [[119]](#ref-119).
- **LEZ**: An account read is reachable through `query_account` and through the
  ...
  string for both not-found and failure [[120]](#ref-120).
...
- **Logos L1**: The balance route is scoped to keys in the node's own wallet
  configuration [[119]](#ref-119), so an integrator cannot read an arbitrary
  address's balance, and a 404 does not distinguish an unknown key from a zero
  balance.
- **LEZ**: The reachable balance read is wallet-scoped: `get_balance` takes a
  ...
  with no block identifier [[120]](#ref-120). The sequencer's
  `getAccountBalance` for an arbitrary address has no FFI export, so a
  third-party balance is available only by reading the indexer's account record.
...
| Stellar  | `getTransaction` [[27]](#ref-27)                                          |
| NEAR     | `tx` [[34]](#ref-34)                                                      |
| Sui      | `sui_getTransactionBlock` [[17]](#ref-17)                                 |
| Logos L1 | `GET /cryptarchia/transaction/:id` [[19]](#ref-19)                        |
| LEZ      | `getTransaction`, indexer `getTransaction` [[21]](#ref-21)[[22]](#ref-22) |
...
assumption to steal money from the institution" [[103]](#ref-103). Under partial
payments the delivered amount can be less than `Amount`, and the correct field
to read is `delivered_amount` in the transaction metadata [[103]](#ref-103). The
field was renamed to `DeliverMax` "to make the field name more specific to its
behavior and help prevent the misunderstandings and exploit described below"
[[103]](#ref-103). The change shipped behind an API version rather than in
place: API v1 continues to render `Amount`, API v2 renders `DeliverMax`
[[103]](#ref-103).

**Potential gaps.**

- **Logos L1**: The route returns the transaction body but no block id, slot, or
  confirmation depth [[119]](#ref-119), so a caller learns that it exists but
  not where it landed.
- **LEZ**: Only the indexer path reaches an application: `query_transaction`
  returns the transaction without its block id [[120]](#ref-120). The
  sequencer's variant, which pairs the transaction with its block id, has no FFI
  export, so the richer of the two results is unreachable.
...
| Stellar  | `simulateTransaction` [[27]](#ref-27)                          |
| NEAR     | `query` with `call_function` [[13]](#ref-13)                   |
| Sui      | `sui_devInspectTransactionBlock` [[17]](#ref-17)               |
...
instead of `O(n)`" [[98]](#ref-98). The same document gives a concrete overfetch
example: `totalDifficulty` is stored separately from the block header, "and many
callers do not require this field. However, every call to `eth_getBlock` still
retrieves this field, requiring a separate disk read, because the RPC server has
no way of knowing if the user requires this field or not" [[98]](#ref-98).
...
- **LEZ**: Programs are deployed and executed, and a state-diff computation runs
  one against state without applying it [[119]](#ref-119), but nothing on the
  RPC surface reaches it.
...
| Stellar  | [NOT FOUND] [[27]](#ref-27)                                                    |
| NEAR     | `validators` [[13]](#ref-13)                                                   |
| Sui      | `suix_getCommitteeInfo`, `suix_getLatestSuiSystemState` [[17]](#ref-17)        |
| Logos L1 | [NOT FOUND]; SDP routes list Blend providers, not validators [[119]](#ref-119) |
| LEZ      | [NOT FOUND] [[21]](#ref-21)                                                    |
...
- **Logos L1**: The SDP routes return Blend mixnet service providers, the
  service-type enumeration having a single variant [[119]](#ref-119);
  Cryptarchia leadership is a private lottery, so there is no committee to
  enumerate.
...
| Stellar  | [NOT FOUND] on Stellar RPC; sequence comes from account data [[27]](#ref-27) |
| NEAR     | `query` with `view_access_key` [[13]](#ref-13)                               |
| Sui      | `suix_getReferenceGasPrice`, `suix_getCoins` [[17]](#ref-17)                 |
| Logos L1 | `GET /mantle/gas-prices` only [[19]](#ref-19)                                |
| LEZ      | `getAccountsNonces` [[21]](#ref-21)                                          |
...
relayers and crypto exchanges" [[91]](#ref-91).
...
even before its submission" [[90]](#ref-90). Durable nonces exist to replace the
expiring parameter with a stored one, which the same guide ties to
multi-signature collection rounds where "one party signs a transaction, and
others may confirm at a later time" [[90]](#ref-90).
...
with the parameter rather than having to compute it [[25]](#ref-25).
...
- **LEZ**: No FFI exposes a nonce or other construction parameter as such.
  ...
  expiry or validity window [[120]](#ref-120). Nonce selection stays inside the
  wallet core, reachable only by submitting through the transfer functions.
...
| Stellar  | [NOT FOUND] as a node method [[27]](#ref-27)                                   |
| NEAR     | [NOT FOUND] as a node method [[13]](#ref-13)                                   |
| Sui      | `unsafe_moveCall`, `unsafe_transferObject`, `unsafe_publish` [[17]](#ref-17)   |
| Logos L1 | [NOT FOUND]; construction is client side [[19]](#ref-19)                       |
| LEZ      | [NOT FOUND]; construction is client side [[21]](#ref-21)                       |
...
request" [[108]](#ref-108). The same comment tells SDK users with native
serialisation support that they "do not need to use this endpoint"
[[108]](#ref-108). The endpoint is enabled by default, so the protection is
documentation rather than configuration [[108]](#ref-108).
...
- **Logos L1**: Construction is client side, matching most of the survey;
  ...
  from the node's own wallet and requires funding keys the node holds
  [[119]](#ref-119).
...
| Stellar  | [NOT FOUND] as a node method [[27]](#ref-27)                                      |
| NEAR     | [NOT FOUND] as a node method [[13]](#ref-13)                                      |
| Sui      | [NOT FOUND] as a node method [[17]](#ref-17)                                      |
| Logos L1 | [NOT FOUND] as a node method [[19]](#ref-19)                                      |
| LEZ      | [NOT FOUND] as a node method [[21]](#ref-21)                                      |
...
- **Logos L1**: No encode or decode method; the codec traits live in a separate
  crate and are never re-exported through a handler [[119]](#ref-119), so a
  non-Rust client must reimplement the layout to compute a hash or verify what
  it signed.
- **LEZ**: Borsh codecs exist as library methods but none is an RPC method; the
  ...
  words and has no decode counterpart [[119]](#ref-119).
...
| Stellar  | `simulateTransaction` [[39]](#ref-39)                                  |
| NEAR     | [NOT FOUND] [[13]](#ref-13)                                            |
| Sui      | `sui_dryRunTransactionBlock` [[17]](#ref-17)                           |
| Logos L1 | [NOT FOUND] [[19]](#ref-19)                                            |
| LEZ      | [NOT FOUND] [[21]](#ref-21)                                            |
...
a transaction without actually submitting it to the network" [[39]](#ref-39).
Returning the transaction data the caller then submits makes simulation a
construction step rather than an optional check.
...
| What the caller learns     | Where it appears                                                                                                                      |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| Did it succeed             | Stellar `results` [[39]](#ref-39); Sui execution effects [[17]](#ref-17)                                                              |
| What it would cost         | Stellar `cost`, `minResourceFee` [[39]](#ref-39); Solana compute units consumed [[25]](#ref-25); Sui gas cost summary [[17]](#ref-17) |
| What state would change    | Stellar `stateChanges`, `events` [[39]](#ref-39)                                                                                      |
| Data to submit with        | Stellar `transactionData` [[39]](#ref-39)                                                                                             |
| Prerequisite step required | Stellar `restorePreamble` [[39]](#ref-39)                                                                                             |
| Basis of the answer        | Stellar `latestLedger` [[39]](#ref-39)                                                                                                |
...
- **LEZ**: A validation routine returns a state diff without applying it, but
  its only caller is the block builder [[119]](#ref-119), so the primitive a
  simulation method would wrap exists and is unreachable over RPC.
...
| Stellar  | `simulateTransaction` returns `cost` and `minResourceFee` [[39]](#ref-39) |
| NEAR     | [NOT FOUND]; `gas_price` is a price, not a usage estimate [[13]](#ref-13) |
| Sui      | `sui_dryRunTransactionBlock` returns a gas cost summary [[17]](#ref-17)   |
| Logos L1 | [NOT FOUND] [[19]](#ref-19)                                               |
| LEZ      | [NOT FOUND] [[21]](#ref-21)                                               |
...
- **Logos L1**: No endpoint returns a transaction's cost; the two-dimensional
  calculation lives in the core library [[119]](#ref-119), and because execution
  gas depends on proof length a client must reimplement that accounting to
  predict a fee.
...
base fee was introduced so that wallets could "auto-set gas fees reliably"
[[41]](#ref-41).

**Potential gaps.**

- **Logos L1**: `/mantle/gas-prices` returns two input prices and the tip they
  were read at [[119]](#ref-119), with no fee for a given transaction or size,
  and no history or percentile from which to choose a priority fee.
- **LEZ**: No fee model exists to price: the only fee in the system is the L1
  cost the sequencer pays to publish [[119]](#ref-119).
...
| NEAR     | [NOT FOUND] as a node method [[13]](#ref-13)                                     |
| Sui      | [NOT FOUND] as a node method [[17]](#ref-17)                                     |
| Logos L1 | `POST /wallet/sign/ed25519`, `POST /wallet/sign/zk` [[19]](#ref-19)              |
| LEZ      | [NOT FOUND] as a node method; signing is client side [[21]](#ref-21)             |
...
[[109]](#ref-109). Node-held keys mean a node cannot safely log its own request
payloads. Bitcoin Core has kept its wallet RPCs but is separating the wallet
into its own process, describing the monolithic structure as carrying "increased
security risks due to the tight integration of components" [[110]](#ref-110).
...
- **LEZ**: Signing is client side, and the wallet FFI offers no signature-only
  export, so every exported path builds, signs, and submits in one call
  [[119]](#ref-119).
...
| Stellar  | [NOT FOUND] as a node method [[27]](#ref-27)                    |
| NEAR     | [NOT FOUND] as a node method [[13]](#ref-13)                    |
| Sui      | [NOT FOUND] as a signing method [[17]](#ref-17)                 |
| Logos L1 | [NOT FOUND] as a distinct message-signing route [[19]](#ref-19) |
| LEZ      | [NOT FOUND] [[21]](#ref-21)                                     |
...
[[87]](#ref-87). NEAR reached the same conclusion independently in NEP-413,
which states the hazard directly: "An attacker could make the user inadvertently
sign a valid transaction which, once signed, could be submitted into the network
to execute it" [[92]](#ref-92). Both solve it with a prefix tag that makes the
payload invalid under the chain's transaction encoding, and NEP-413 adds a nonce
because "including a nonce helps to mitigate replay attacks, in which an
attacker can delay or re-send a signed message" [[92]](#ref-92).
...
items that make up the message" [[88]](#ref-88).
...
- **Logos L1**: There is no separate message-signing route, but both signing
  ...
  and sign it unchecked [[119]](#ref-119), so they sign any digest under a
  node-held key without the domain separation ERC-191 and NEP-413 describe.
...
| Stellar  | [NOT FOUND] as a node method [[27]](#ref-27)                   |
| NEAR     | [NOT FOUND] as a node method [[13]](#ref-13)                   |
| Sui      | `SignatureVerificationService.VerifySignature` [[15]](#ref-15) |
| Logos L1 | [NOT FOUND] as a client-facing verify method [[19]](#ref-19)   |
| LEZ      | [NOT FOUND] as a client-facing verify method [[21]](#ref-21)   |
...
- **Logos L1**: No client-facing verify method; verification exists per
  operation type inside the ledger [[119]](#ref-119) and is not reachable from
  the API.
- **LEZ**: No client-facing verify method; signature validation is a library
  function reached only through submission's stateless check [[119]](#ref-119).
...
| Stellar  | `sendTransaction` [[27]](#ref-27)                                                        |
| NEAR     | `send_tx`, `broadcast_tx_async` [[34]](#ref-34)                                          |
| Sui      | `sui_executeTransactionBlock` [[17]](#ref-17)                                            |
| Logos L1 | `POST /mempool/add/tx`, returns no identifier [[119]](#ref-119)                          |
| LEZ      | `sendTransaction` [[21]](#ref-21)                                                        |

**Why it exists.** No specific or relevant context has been found.

Solana's documentation warns that "a successful response doesn't guarantee the
transaction will be processed or confirmed" [[42]](#ref-42).

**What comes back.** Responses range from a bare identifier to a full result:

| What the caller learns | Where it appears                                                                                       |
| ---------------------- | ------------------------------------------------------------------------------------------------------ |
| Transaction identifier | Cosmos `txhash` [[93]](#ref-93); Solana the first signature [[42]](#ref-42)                            |
| Accepted or rejected   | Cosmos `code`, `codespace` [[93]](#ref-93); XRPL `engine_result`, `engine_result_code` [[26]](#ref-26) |
| Human-readable reason  | Cosmos `raw_log`, `info` [[93]](#ref-93); XRPL `engine_result_message` [[26]](#ref-26)                 |
| Where it landed        | Cosmos `height`, `timestamp` [[93]](#ref-93)                                                           |
| Resource usage         | Cosmos `gas_wanted`, `gas_used` [[93]](#ref-93)                                                        |
| Effects                | Cosmos `events`, `logs` [[93]](#ref-93)                                                                |
| Echo of the submission | Cosmos `tx` [[93]](#ref-93); XRPL `tx_blob`, `tx_json` [[26]](#ref-26)                                 |
...
May be non-deterministic" [[93]](#ref-93), so it is not safe to parse. XRPL's
engine result is a provisional outcome rather than a ledger result
[[26]](#ref-26), and Solana warns that "a successful response doesn't guarantee
the transaction will be processed or confirmed" [[42]](#ref-42).

**Potential gaps.**

- **Logos L1**: The submission route returns an empty body [[119]](#ref-119), so
  unlike every surveyed chain it yields no identifier to track the transaction
  by, while the Blend route does return one [[119]](#ref-119).
- **LEZ**: `sendTransaction` returns the transaction hash but no acceptance
  code, position, or reason [[21]](#ref-21), so a caller learns only that the
  node accepted it into a queue.
...
| Stellar  | `simulateTransaction` [[39]](#ref-39)                                          |
| NEAR     | [NOT FOUND] [[34]](#ref-34)                                                    |
| Sui      | `sui_dryRunTransactionBlock` [[17]](#ref-17)                                   |
| Logos L1 | [NOT FOUND]; validation happens on submission [[19]](#ref-19)                  |
| LEZ      | [NOT FOUND] as a standalone method [[21]](#ref-21)                             |
...
- **LEZ**: No standalone check, though submission runs a real one and reports it
  with a reason [[119]](#ref-119); the state-level check that would predict
  execution failure is not reachable over RPC [[119]](#ref-119).
...
| Stellar  | [NOT FOUND]; batching is within a transaction's operations [[27]](#ref-27)           |
| NEAR     | [NOT FOUND]; batching is within a transaction's actions [[13]](#ref-13)              |
| Sui      | `unsafe_batchTransaction` builds a batch rather than submitting many [[17]](#ref-17) |
| Logos L1 | [NOT FOUND] [[19]](#ref-19)                                                          |
| LEZ      | [NOT FOUND] [[21]](#ref-21)                                                          |
...
transactions" [[89]](#ref-89). A low-fee parent is rejected before the child
that would pay for it can be seen.
...
- **LEZ**: No batch submission, and no in-transaction batching either: a public
  transaction carries one program invocation [[115]](#ref-115), so LEZ lacks
  both forms the survey found elsewhere.
...
| Cosmos   | `GetTx`, `/tx` [[29]](#ref-29)[[9]](#ref-9)                          |
| Stellar  | `getTransaction` [[27]](#ref-27)                                     |
| NEAR     | `tx`, `EXPERIMENTAL_tx_status` [[34]](#ref-34)                       |
| Sui      | `sui_getTransactionBlock` [[17]](#ref-17)                            |
| Logos L1 | `POST /mantle/status`, `Unknown` or `Pending` only [[119]](#ref-119) |
| LEZ      | status projected from `getTransaction` [[21]](#ref-21)               |
...
- **Logos L1**: Status reports only `Unknown` or `Pending` [[119]](#ref-119),
  documented as best-effort, so `Unknown` conflates never-seen, mined, and
  evicted; the response is positional with no hash echoed back, and its declared
  OpenAPI type differs from what the service returns [[119]](#ref-119).
- **LEZ**: The reachable status call is `poll_transaction_status`, which returns
  a bare boolean meaning found or not found [[120]](#ref-120), conflating
  never-seen, pending, and rejected. The `Pending`, `Safe`, and `Finalized`
  marker lives on the block [[115]](#ref-115), so a confirmation level costs a
  transaction fetch and then a block fetch on the separate indexer stack.
...
| Stellar  | [NOT FOUND]; polling `getTransaction` is documented [[27]](#ref-27) |
| NEAR     | `wait_until` on `send_tx` and `tx` [[34]](#ref-34)                  |
| Sui      | request type on `sui_executeTransactionBlock` [[17]](#ref-17)       |
| Logos L1 | [NOT FOUND] [[19]](#ref-19)                                         |
| LEZ      | [NOT FOUND]; the module polls in a loop [[21]](#ref-21)             |
...
wait when broadcasting tx" [[112]](#ref-112). The docs now describe the older
pair as kept "for backward compatibility but offer less control over when the
call returns" [[34]](#ref-34). Note also that NEAR distinguishes refund receipts
from non-refund receipts across its levels, because refunds settle later and
most callers do not need to wait for them [[34]](#ref-34).
...
the same thing. This is semantically unsound and confusion" [[111]](#ref-111).
The surviving `safe` tag is defined in the schema as "The most recent block that
is safe from re-orgs under honest majority and certain synchronicity
assumptions" [[11]](#ref-11).
...
- **LEZ**: No FFI exposes a wait. The retry and delay budget lives inside the
  ...
  `poll_transaction_status` itself [[120]](#ref-120), and its boolean return
  offers no confirmation level to wait on.
...
| Stellar  | [NOT FOUND] [[27]](#ref-27)                                              |
| NEAR     | [NOT FOUND] [[13]](#ref-13)                                              |
| Sui      | [NOT FOUND] [[17]](#ref-17)                                              |
| Logos L1 | `GET /mempool/view` [[19]](#ref-19)                                      |
| LEZ      | [NOT FOUND] [[21]](#ref-21)                                              |
...
- **LEZ**: The sequencer holds a bounded mempool but the service uses its handle
  only to push [[119]](#ref-119); the only external visibility is a metrics
  gauge, so a submitted transaction is unobservable until it appears in a block.
...
| ---------------------- | ------------------------------------- | ------------------------------------- |
| Did it succeed         | `root` [[100]](#ref-100)              | before the Byzantium upgrade          |
| Did it succeed         | `status` [[100]](#ref-100)            | after the Byzantium upgrade           |
| What it cost           | `effectiveGasPrice` [[100]](#ref-100) | defined against the EIP-1559 base fee |
| What it cost           | `blobGasUsed` [[100]](#ref-100)       | blob transactions only, per EIP-4844  |
| Cumulative block cost  | `cumulativeGasUsed` [[100]](#ref-100) | always                                |
...
specified for transactions included after the Byzantium upgrade"
[[100]](#ref-100).
...
requires a great deal of raw string matching and parsing" [[106]](#ref-106).
...
- **Logos L1**: Events are typed and carry a transaction hash per entry, but
  ...
  [[119]](#ref-119), so ordinary transfers produce no effects record and there
  is no success or failure indicator.
- **LEZ**: No effects view; the nearest is the post-state account snapshot
  inside a privacy-preserving transaction's public actions [[115]](#ref-115),
  which is a snapshot rather than a delta and absent from public transactions.
...
sync with reality" [[96]](#ref-96). The sharper point concerns empty results: if
the response is an empty array, "the client is in a situation where they don't
know what block the results are for", and "there is no decision the client can
make that allows them a guarantee of recovery" [[96]](#ref-96). A response must
carry enough context to identify the position it describes.
...
which would only result on timeouts and CPU abuse" [[46]](#ref-46). The same
query can therefore succeed against one client and fail against another.
...
is queried, the balances may not reconcile" [[95]](#ref-95). The EIP explicitly
rejects solving this with a stateful subscription, on the grounds that it
"requires a persistent connection between the client and node" and "increases
coupling between the client and the node" [[95]](#ref-95).
...
[[97]](#ref-97). The design lesson for an API is that retention should be
declared, not discovered through a failed request.
...
- **Logos L1**: A tip parameter pins three routes, but two are wallet-scoped
  [[119]](#ref-119), so pinned reads are unavailable for third-party state; no
  route declares a retention floor.
- **LEZ**: No FFI exposes a pinned read. `getAccountAtBlock` exists on the
  indexer RPC but is not among the eight exported queries [[120]](#ref-120), so
  every account read an application can make is against current state, and
  neither stack declares a retention floor.
...
RPC call would execute an unfiltered query)" [[104]](#ref-104). The caller
received a wrong answer and the node did the most expensive possible work. An
invalid query should be rejected loudly rather than falling back to a permissive
default.

Three idioms are visible: an opaque marker whose "value is stable even if there
is a change in the server's range of available ledgers" (XRPL) [[47]](#ref-47),
a reusable protobuf pagination message shared by every module query (Cosmos)
[[49]](#ref-49), and HAL hypermedia links (Stellar Horizon) [[50]](#ref-50). LEZ
uses both a cursor and an offset idiom in one interface.
...
- **Logos L1**: No cursor is exposed; the headers route silently truncates at
  ...
  limit from the requested slot range with no server cap [[119]](#ref-119).
- **LEZ**: Two idioms coexist, a cursor on `getBlocks` and a numeric offset on
  `getTransactionsByAccount` [[22]](#ref-22), and neither returns a total, a
  next position, or an indication that more remain.
...
- **LEZ**: `getTransactionsByAccount` covers this on the indexer
  [[22]](#ref-22), though offset paging over a growing set can skip or repeat
  entries as new transactions land ahead of the offset.
...
| Stellar  | `cursor` on streaming mode [[48]](#ref-48)                                   |
| NEAR     | [NOT FOUND]; no streaming surface [[13]](#ref-13)                            |
| Sui      | [NOT FOUND] on the documented subscription methods [[15]](#ref-15)           |
| Logos L1 | `slot_from` on `/cryptarchia/blocks_range` [[84]](#ref-84)                   |
| LEZ      | [NOT FOUND]; `subscribeToFinalizedBlocks` takes no arguments [[22]](#ref-22) |
...
reestablish the connection" [[66]](#ref-66). WebSocket has no equivalent
standard mechanism, so every chain that streams over WebSocket must invent its
own cursor, and most have not. Stellar's resumable stream is served over SSE
from Horizon [[48]](#ref-48)[[59]](#ref-59).
...
restart from a chosen slot [[84]](#ref-84). Its unbounded live stream
...
- **Logos L1**: `/cryptarchia/blocks_range` accepts slot bounds with a
  batch-size hint [[84]](#ref-84), better supported than most of the survey; the
  live stream accepts no position, so resuming means switching routes and
...
- **LEZ**: There is no stream to resume, because no FFI exposes a subscription.
  ...
  calls [[120]](#ref-120), with the consumer holding its own position
  throughout.
...
| Stellar  | JSON-RPC 2.0 error object [[43]](#ref-43)               | JSON-RPC codes [[43]](#ref-43)                            |
| NEAR     | JSON-RPC 2.0 error object [[43]](#ref-43)               | JSON-RPC codes [[43]](#ref-43)                            |
| Sui      | JSON-RPC and gRPC status [[43]](#ref-43)[[54]](#ref-54) | JSON-RPC and gRPC codes [[43]](#ref-43)[[54]](#ref-54)    |
| Logos L1 | HTTP status with an error body [[19]](#ref-19)          | [NOT FOUND] as a defined taxonomy                         |
| LEZ      | JSON-RPC error object [[21]](#ref-21)                   | standard codes only, no application codes [[55]](#ref-55) |
...
numeric JSON-RPC codes, despite serving JSON-RPC [[53]](#ref-53).
...
implementations" [[99]](#ref-99). EIP-1898 supplies the design rule that the
taxonomy itself does not state: two different failure causes must not share a
code, because the caller's recovery differs, so a block-not-found error "should
be different than the error code for the block not found case so that the caller
can distinguish the cases" [[95]](#ref-95).
...
API error (-32003)" [[104]](#ref-104). Transport failures and execution outcomes
are different things, and moving one to the other is a breaking change.
...
[[4]](#ref-4). XRPL's gRPC surface exists for Clio to extract data from the node
rather than as a public client API [[7]](#ref-7).
...
protocol", explicitly transport agnostic [[43]](#ref-43). It supports batching
and notifications. It defines no schema and says nothing about streaming, so
every subscription mechanism in this survey is a per-chain extension layered on
a full-duplex transport rather than a JSON-RPC feature. OpenRPC fills the schema
gap, built on JSON Schema with discovery via `rpc.discover` [[61]](#ref-61).

**REST** is browser and cache friendly. Cosmos states its REST layer exists so
web applications can reach the same functionality "without requiring HTTP2"
[[8]](#ref-8). It has no native streaming, so Horizon bolts on SSE. It need not
be hand written: Cosmos generates it from protobuf via gRPC-gateway, and "for
each gRPC endpoint defined in a Protobuf Query service, the Cosmos SDK offers a
REST equivalent" [[8]](#ref-8).
...
Java, Go, Python, Ruby, C++, C#, Dart, Objective-C, PHP, and Rust
[[62]](#ref-62). Streaming is first class: server streaming means "the client
sends a request to the server and gets a stream to read a sequence of messages
back" [[63]](#ref-63). It is the only transport here where push is part of the
core protocol. Its cost is browser reach, which is precisely why Cosmos
maintains a REST facade.

**GraphQL** covers "a type system, query language and execution semantics,
static validation, and type introspection" [[64]](#ref-64). Geth's own
justification for serving it is that it avoids "the extra load on the client for
filling in fields which are not needed" and allows "combining several
traditional JSON-RPC requests into one query" [[57]](#ref-57). The specification
defines `subscription` as an operation type but binds it to no transport
[[64]](#ref-64)[[65]](#ref-65), and neither Geth's nor Sui's GraphQL surface
documents subscriptions. In both Sui and Stellar, the rich-query surface sits on
the indexer rather than the node.

**WebSocket** is full duplex, and Geth's constraint is the general rule:
"Subscriptions require a full duplex connection" [[51]](#ref-51). It is the de
facto subscription transport across Ethereum, Solana, XRPL, and Cosmos.

**SSE** is unidirectional server to client, using `text/event-stream`
[[66]](#ref-66). Its decisive advantage for ledger tailing is built-in
resumption: the `Last-Event-ID` header "reports an EventSource object's last
event ID string to the server when the user agent is to reestablish the
connection" [[66]](#ref-66). WebSocket has no equivalent standard mechanism, so
each chain must invent its own cursor. This is the mechanical reason stream
resumption is so poorly supported across the survey.
...
decommission in mid-October 2026 [[16]](#ref-16)[[18]](#ref-18). The split
rationale is "gRPC for real-time full node access, archival service access and
transaction execution, and GraphQL RPC for rich, structured queries"
[[16]](#ref-16).
...
"SubscribeEvents with an EventFilter replaces sui_subscribeEvent"
[[16]](#ref-16). A chain moving push off WebSocket and onto gRPC streaming, with
server-side filtering as the stated gain.
...
RPC nodes" [[11]](#ref-11). It is a deliberate node and indexer split.
...
Different use cases need different things" [[107]](#ref-107). The same post
gives the retention argument for a separate archival service: "Full nodes prune
data to stay performant. That's fine for most operations, but it makes deep
historical queries difficult or impossible without running a dedicated full node
yourself" [[107]](#ref-107). The migration guide concedes the cost of replacing
one interface with two: "Most exchanges and indexers end up using both: gRPC for
the live data path and transaction submission, GraphQL for ad hoc queries"
[[16]](#ref-16).
...
with a body pointing at v2 while leaving invalid v1 paths at 404
[[113]](#ref-113). Solana removed fifteen named endpoints in one major version,
with the stated reasoning that the server "is carrying around a lot of obsolete
and deprecated endpoints. v2.0 means it is time to remove them"
[[114]](#ref-114). No project in the survey has published a retrospective on
what such a removal cost its integrators.
...
library is community maintained [[71]](#ref-71).
...
comment, and the repository sits outside the XRPLF organisation [[75]](#ref-75).
...
Rust-based bdk_wallet library" [[72]](#ref-72). It builds Kotlin, Android, and
Swift targets in-repo, and Kotlin JVM, Python, Dart, and React Native TypeScript
are maintained as separate downstream repositories consuming the same binding
layer [[72]](#ref-72). One Rust core therefore reaches six language targets,
although only the first group is produced by the binding repository itself.
Notably it comes from a community project rather than a foundation, and the BDK
core repository does not mention the bindings at all.
...
in various languages" [[116]](#ref-116). Mesh makes generation a property of the
specification, noting that requests and responses "can be crafted with
auto-generated code using Swagger Codegen or OpenAPI Generator"
[[117]](#ref-117).
...
under the /swagger route on the API server" [[8]](#ref-8). This is why Cosmos
publishes so few named per-language SDKs: the protobuf contract plus each
language's own gRPC toolchain substitutes for them. Sui similarly publishes
protobuf interface definitions usable to generate client libraries
[[60]](#ref-60).
...
for writing smart contracts" [[82]](#ref-82). Its client SDKs are simply listed
with their maintainers, several of them community-run [[78]](#ref-78), which is
consistent with hand-writing without arguing for it.

### 3.3 Deprecations

Sui's JSON-RPC retirement cascades into its SDKs, which is why it ships a new
Rust SDK that "does not support JSON RPC" alongside a legacy one that does
[[80]](#ref-80).
...
for new projects" [[67]](#ref-67).

Solana renamed rather than deprecated: the 2.x line of `@solana/web3.js` became
`@solana/kit` [[73]](#ref-73), and the earlier v1 line, wallet adapter, and
framework kit packages "are superseded. New apps build directly on
`@solana/kit`" [[83]](#ref-83).
...
[Appendix: Logos API Surfaces](./logos-api-surfaces.md) [[120]](#ref-120).
...
| ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| Get a balance                 | L1 scoped to node-custodial keys, not arbitrary subjects [[19]](#ref-19)                                                                       |
| Get transaction status        | L1 reports mempool status only, no lifecycle or finality [[19]](#ref-19)                                                                       |
| Get execution effects         | L1 block scoped, no per-transaction effects view [[19]](#ref-19); LEZ absent [[21]](#ref-21)                                                   |
| Paginate a result set         | LEZ mixes a cursor idiom and an offset idiom in one interface [[22]](#ref-22)                                                                  |
| Subscribe to new blocks       | Live streams accept no start position on either target; L1 offers a separate bounded range route [[19]](#ref-19)[[84]](#ref-84)[[22]](#ref-22) |
| Structured errors             | LEZ uses stock JSON-RPC codes with free text, no taxonomy [[55]](#ref-55)                                                                      |
| Query historical transactions | LEZ indexer only, offset paginated [[22]](#ref-22)                                                                                             |
| Estimate the fee or fee rate  | L1 serves gas-price inputs, not an estimate for given bytes [[19]](#ref-19)                                                                    |
...
L1 ships a C ABI generated by cbindgen [[81]](#ref-81). The header exports ten
symbols, of which four are API operations: `get_cryptarchia_info`,
`get_balance`, `subscribe_to_new_blocks`, and `transfer_funds`. The remainder
are node lifecycle (`start_lb_node`, `stop_node`), memory management, and a
status helper. Against roughly forty HTTP routes on the node [[19]](#ref-19),
the binding is oriented around embedding and running a node rather than
consuming the node API as a client. `subscribe_to_new_blocks` is callback based
and carries no resume anchor [[81]](#ref-81).

## References

001. <a id="ref-1"></a>Go Ethereum, "JSON-RPC Server" documentation.
     https://geth.ethereum.org/docs/interacting-with-geth/rpc
002. <a id="ref-2"></a>Ethereum, "execution-apis" specification repository.
     https://github.com/ethereum/execution-apis
003. <a id="ref-3"></a>Bitcoin Core, "JSON-RPC Interface" documentation.
     https://raw.githubusercontent.com/bitcoin/bitcoin/master/doc/JSON-RPC-interface.md
004. <a id="ref-4"></a>Bitcoin Core, "Unauthenticated REST Interface"
     documentation.
     https://github.com/bitcoin/bitcoin/blob/master/doc/REST-interface.md
005. <a id="ref-5"></a>Solana, "RPC API" documentation.
     https://solana.com/docs/rpc
006. <a id="ref-6"></a>XRPL, "HTTP and WebSocket APIs" reference.
     https://xrpl.org/docs/references/http-websocket-apis
007. <a id="ref-7"></a>XRPL, "xrp_ledger.proto" gRPC service definition
     (rippled).
     https://raw.githubusercontent.com/XRPLF/rippled/develop/include/xrpl/proto/org/xrpl/rpc/v1/xrp_ledger.proto
008. <a id="ref-8"></a>Cosmos SDK, "gRPC, REST, and CometBFT Endpoints"
     documentation, v0.50.
     https://docs.cosmos.network/sdk/v0.50/learn/advanced/grpc_rest
009. <a id="ref-9"></a>CometBFT, "RPC OpenAPI specification".
     https://raw.githubusercontent.com/cometbft/cometbft/main/rpc/openapi/openapi.yaml
010. <a id="ref-10"></a>Cosmos SDK,
...
011. <a id="ref-11"></a>Stellar, "Stellar RPC" documentation.
     https://developers.stellar.org/docs/data/apis/rpc
012. <a id="ref-12"></a>Stellar, "stellar-rpc.openrpc.json" specification
     document.
     https://raw.githubusercontent.com/stellar/stellar-docs/main/static/stellar-rpc.openrpc.json
013. <a id="ref-13"></a>NEAR, "RPC API" documentation.
     https://docs.near.org/api/rpc/introduction
014. <a id="ref-14"></a>NEAR, "nearcore JSON-RPC OpenAPI specification".
     https://raw.githubusercontent.com/near/nearcore/master/chain/jsonrpc/openapi/openapi.json
015. <a id="ref-15"></a>Sui, "Full Node Protocol" reference.
     https://docs.sui.io/references/fullnode-protocol
016. <a id="ref-16"></a>Sui, "JSON-RPC Migration" documentation.
     https://docs.sui.io/develop/accessing-data/json-rpc-migration
017. <a id="ref-17"></a>Sui, "Sui JSON-RPC OpenRPC specification", version
     1.80.0.
     https://raw.githubusercontent.com/MystenLabs/sui/main/crates/sui-open-rpc/spec/openrpc.json
018. <a id="ref-18"></a>Sui, "GraphQL RPC" documentation.
     https://docs.sui.io/concepts/graphql-rpc
019. <a id="ref-19"></a>logos-blockchain, `nodes/api-common/src/paths.rs` and
     `nodes/node/binary/src/api/backend.rs`, commit `ecb2cc6`.
     https://github.com/logos-blockchain/logos-blockchain
020. <a id="ref-20"></a>logos-blockchain, `nodes/node/binary/src/api/openapi.rs`
     and `nodes/node/binary/src/api/backend.rs:215`, commit `ecb2cc6`.
     https://github.com/logos-blockchain/logos-blockchain
021. <a id="ref-21"></a>logos-execution-zone,
     `lez/sequencer/service/rpc/src/lib.rs`, commit `47eba25`.
     https://github.com/logos-blockchain/logos-execution-zone
022. <a id="ref-22"></a>logos-execution-zone,
     `lez/indexer/service/rpc/src/lib.rs`, commit `47eba25`.
     https://github.com/logos-blockchain/logos-execution-zone
023. <a id="ref-23"></a>Ethereum, "JSON-RPC API" documentation.
     https://ethereum.org/en/developers/docs/apis/json-rpc/
024. <a id="ref-24"></a>Bitcoin, "Original Bitcoin client RPC API reference".
     https://developer.bitcoin.org/reference/rpc/
025. <a id="ref-25"></a>Solana, "RPC HTTP Methods" documentation.
     https://solana.com/docs/rpc/http
026. <a id="ref-26"></a>XRPL, "Public API Methods" reference.
     https://xrpl.org/public-api-methods.html
027. <a id="ref-27"></a>Stellar, "Stellar RPC Methods" reference.
     https://developers.stellar.org/docs/data/apis/rpc/api-reference/methods
028. <a id="ref-28"></a>Ethereum, "execution-apis: src/eth/client.yaml".
     https://raw.githubusercontent.com/ethereum/execution-apis/main/src/eth/client.yaml
029. <a id="ref-29"></a>Cosmos SDK, "cosmos/tx/v1beta1/service.proto".
     https://raw.githubusercontent.com/cosmos/cosmos-sdk/main/proto/cosmos/tx/v1beta1/service.proto
030. <a id="ref-30"></a>Ethereum, "execution-apis: src/eth/state.yaml".
     https://raw.githubusercontent.com/ethereum/execution-apis/main/src/eth/state.yaml
031. <a id="ref-31"></a>Cosmos SDK, "cosmos/auth/v1beta1/query.proto".
     https://raw.githubusercontent.com/cosmos/cosmos-sdk/main/proto/cosmos/auth/v1beta1/query.proto
032. <a id="ref-32"></a>Cosmos SDK, "cosmos/bank/v1beta1/query.proto".
     https://raw.githubusercontent.com/cosmos/cosmos-sdk/main/proto/cosmos/bank/v1beta1/query.proto
033. <a id="ref-33"></a>Ethereum, "execution-apis: src/eth/transaction.yaml".
     https://raw.githubusercontent.com/ethereum/execution-apis/main/src/eth/transaction.yaml
034. <a id="ref-34"></a>NEAR, "RPC: Transactions" documentation.
     https://docs.near.org/api/rpc/transactions
035. <a id="ref-35"></a>Ethereum, "execution-apis: src/eth/execute.yaml".
     https://raw.githubusercontent.com/ethereum/execution-apis/main/src/eth/execute.yaml
036. <a id="ref-36"></a>Ethereum, "execution-apis: src/eth/submit.yaml".
     https://raw.githubusercontent.com/ethereum/execution-apis/main/src/eth/submit.yaml
037. <a id="ref-37"></a>Bitcoin, "BIP-174: Partially Signed Bitcoin Transaction
     Format". https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki
038. <a id="ref-38"></a>XRPL, "simulate" transaction method.
     https://xrpl.org/docs/references/http-websocket-apis/public-api-methods/transaction-methods/simulate
039. <a id="ref-39"></a>Stellar, "simulateTransaction" method reference.
     https://developers.stellar.org/docs/data/apis/rpc/api-reference/methods/simulateTransaction
040. <a id="ref-40"></a>Ethereum, "execution-apis: src/eth/fee_market.yaml".
     https://raw.githubusercontent.com/ethereum/execution-apis/main/src/eth/fee_market.yaml
041. <a id="ref-41"></a>Ethereum, "EIP-1559: Fee market change for ETH 1.0
     chain". https://eips.ethereum.org/EIPS/eip-1559
042. <a id="ref-42"></a>Solana, "sendTransaction" RPC method.
     https://solana.com/docs/rpc/http/sendtransaction
043. <a id="ref-43"></a>JSON-RPC Working Group, "JSON-RPC 2.0 Specification".
     https://www.jsonrpc.org/specification
044. <a id="ref-44"></a>Solana, "getSignatureStatuses" RPC method.
     https://solana.com/docs/rpc/http/getsignaturestatuses
045. <a id="ref-45"></a>Solana, "RPC WebSocket Methods" documentation.
     https://solana.com/docs/rpc/websocket
046. <a id="ref-46"></a>Ethereum, "execution-apis: src/eth/filter.yaml".
     https://raw.githubusercontent.com/ethereum/execution-apis/main/src/eth/filter.yaml
047. <a id="ref-47"></a>XRPL, "account_tx" account method.
     https://xrpl.org/docs/references/http-websocket-apis/public-api-methods/account-methods/account_tx
048. <a id="ref-48"></a>Stellar, "List All Effects" Horizon reference.
     https://developers.stellar.org/docs/data/apis/horizon/api-reference/list-all-effects
049. <a id="ref-49"></a>Cosmos SDK,
     "cosmos/base/query/v1beta1/pagination.proto".
     https://raw.githubusercontent.com/cosmos/cosmos-sdk/main/proto/cosmos/base/query/v1beta1/pagination.proto
050. <a id="ref-50"></a>Stellar, "Horizon Pagination" documentation.
     https://developers.stellar.org/docs/data/apis/horizon/api-reference/structure/pagination
051. <a id="ref-51"></a>Go Ethereum, "Publish and Subscribe" documentation.
     https://geth.ethereum.org/docs/interacting-with-geth/rpc/pubsub
052. <a id="ref-52"></a>Bitcoin Core, "ZeroMQ" documentation.
     https://github.com/bitcoin/bitcoin/blob/master/doc/zmq.md
053. <a id="ref-53"></a>XRPL, "Error Formatting" API convention.
     https://xrpl.org/docs/references/http-websocket-apis/api-conventions/error-formatting
054. <a id="ref-54"></a>gRPC, "Status codes and their use in gRPC".
     https://grpc.io/docs/guides/status-codes/
055. <a id="ref-55"></a>logos-execution-zone,
     `lez/indexer/service/src/service.rs:343-349`, commit `47eba25`.
     https://github.com/logos-blockchain/logos-execution-zone
056. <a id="ref-56"></a>Ethereum, "EIP-1193: Ethereum Provider JavaScript API".
     https://eips.ethereum.org/EIPS/eip-1193
057. <a id="ref-57"></a>Go Ethereum, "GraphQL Server" documentation.
     https://geth.ethereum.org/docs/interacting-with-geth/rpc/graphql
058. <a id="ref-58"></a>XRPL, "Request Formatting" API convention.
     https://xrpl.org/docs/references/http-websocket-apis/api-conventions/request-formatting
059. <a id="ref-59"></a>Stellar, "Horizon Streaming" documentation.
     https://developers.stellar.org/docs/data/apis/horizon/api-reference/structure/streaming
060. <a id="ref-60"></a>Sui, "sui-apis" protobuf interface definitions.
     https://github.com/MystenLabs/sui-apis
061. <a id="ref-61"></a>OpenRPC, project homepage. https://open-rpc.org/
062. <a id="ref-62"></a>gRPC, "Introduction to gRPC".
     https://grpc.io/docs/what-is-grpc/introduction/
063. <a id="ref-63"></a>gRPC, "Core concepts, architecture and lifecycle".
     https://grpc.io/docs/what-is-grpc/core-concepts/
064. <a id="ref-64"></a>GraphQL Foundation, "GraphQL specification" repository.
     https://raw.githubusercontent.com/graphql/graphql-spec/main/README.md
065. <a id="ref-65"></a>GraphQL Foundation, "GraphQL over HTTP" specification
     repository. https://github.com/graphql/graphql-over-http
066. <a id="ref-66"></a>WHATWG, "Server-sent events", HTML Living Standard.
     https://html.spec.whatwg.org/multipage/server-sent-events.html
067. <a id="ref-67"></a>Ethereum, "JavaScript API libraries" documentation.
     https://ethereum.org/en/developers/docs/apis/javascript/
068. <a id="ref-68"></a>Ethereum, "Rust developer resources".
     https://ethereum.org/en/developers/docs/programming-languages/rust/
069. <a id="ref-69"></a>Ethereum, "go-ethereum" repository.
     https://github.com/ethereum/go-ethereum
070. <a id="ref-70"></a>Ethereum, "web3.py" repository.
     https://github.com/ethereum/web3.py
071. <a id="ref-71"></a>Bitcoin, "Bitcoin Development" resources page.
     https://bitcoin.org/en/development
072. <a id="ref-72"></a>Bitcoin Dev Kit, "bdk-ffi" repository.
     https://github.com/bitcoindevkit/bdk-ffi
073. <a id="ref-73"></a>Solana, "Solana Clients" documentation.
     https://solana.com/docs/clients
074. <a id="ref-74"></a>Solana, "Rust Client" documentation.
     https://solana.com/docs/clients/rust
075. <a id="ref-75"></a>XRPL, "Client Libraries" reference.
     https://xrpl.org/docs/references/client-libraries
076. <a id="ref-76"></a>Cosmos, "CosmJS" repository.
     https://github.com/cosmos/cosmjs
077. <a id="ref-77"></a>Cosmos, "cosmos-sdk" repository.
     https://github.com/cosmos/cosmos-sdk
078. <a id="ref-78"></a>Stellar, "Client SDKs" documentation.
     https://developers.stellar.org/docs/tools/sdks/client-sdks
079. <a id="ref-79"></a>NEAR, "NEAR API" documentation.
     https://docs.near.org/tools/near-api
080. <a id="ref-80"></a>Sui, "Sui SDKs" reference.
     https://docs.sui.io/references/sui-sdks
081. <a id="ref-81"></a>logos-blockchain, `c-bindings/logos_blockchain.h` and
     `c-bindings/cbindgen.toml`, commit `ecb2cc6`.
     https://github.com/logos-blockchain/logos-blockchain
082. <a id="ref-82"></a>Stellar, "Build Your Own SDK" documentation.
     https://developers.stellar.org/docs/tools/sdks/build-your-own
083. <a id="ref-83"></a>Solana, "web3.js Compatibility" documentation.
     https://solana.com/docs/frontend/web3-compat
084. <a id="ref-84"></a>logos-blockchain, `nodes/api-common/src/paths.rs:33` and
     `nodes/node/binary/src/api/queries.rs`, commit `ecb2cc6`.
     https://github.com/logos-blockchain/logos-blockchain
085. <a id="ref-85"></a>XRPL Foundation, "xrpl-rust" repository.
     https://github.com/XRPLF/xrpl-rust
086. <a id="ref-86"></a>Ethereum, "EIP-155: Simple replay attack protection".
     https://eips.ethereum.org/EIPS/eip-155
087. <a id="ref-87"></a>Ethereum, "ERC-191: Signed Data Standard".
     https://eips.ethereum.org/EIPS/eip-191
088. <a id="ref-88"></a>Ethereum, "EIP-712: Typed structured data hashing and
     signing". https://eips.ethereum.org/EIPS/eip-712
089. <a id="ref-89"></a>Bitcoin, "BIP-331: Ancestor Package Relay".
     https://github.com/bitcoin/bips/blob/master/bip-0331.mediawiki
090. <a id="ref-90"></a>Solana, "Durable and Offline Transaction Signing using
     Nonces".
     https://solana.com/developers/guides/advanced/introduction-to-durable-nonces
091. <a id="ref-91"></a>Cosmos SDK, "ADR-070: Unordered Transactions".
     https://raw.githubusercontent.com/cosmos/cosmos-sdk/main/docs/architecture/adr-070-unordered-account.md
092. <a id="ref-92"></a>NEAR, "NEP-413: Wallet API for signing messages".
     https://github.com/near/NEPs/blob/master/neps/nep-0413.md
093. <a id="ref-93"></a>Cosmos SDK, "cosmos/base/abci/v1beta1/abci.proto".
     https://raw.githubusercontent.com/cosmos/cosmos-sdk/main/proto/cosmos/base/abci/v1beta1/abci.proto
094. <a id="ref-94"></a>Ethereum, "EIP-695: Create eth_chainId method for
     JSON-RPC". https://eips.ethereum.org/EIPS/eip-695
095. <a id="ref-95"></a>Ethereum, "EIP-1898: Add blockHash to defaultBlock
     methods". https://eips.ethereum.org/EIPS/eip-1898
096. <a id="ref-96"></a>Ethereum, "EIP-234: Add blockHash to JSON-RPC filter
     options". https://eips.ethereum.org/EIPS/eip-234
097. <a id="ref-97"></a>Ethereum, "EIP-4444: Bound Historical Data in Execution
     Clients". https://eips.ethereum.org/EIPS/eip-4444
098. <a id="ref-98"></a>Ethereum, "EIP-1767: GraphQL interface to Ethereum node
     data". https://eips.ethereum.org/EIPS/eip-1767
099. <a id="ref-99"></a>Ethereum, "EIP-1474: Remote procedure call
     specification". https://eips.ethereum.org/EIPS/eip-1474
100. <a id="ref-100"></a>Ethereum, "execution-apis: src/schemas/receipt.yaml".
     https://raw.githubusercontent.com/ethereum/execution-apis/main/src/schemas/receipt.yaml
101. <a id="ref-101"></a>XRPL, "server_info" method reference.
...
102. <a id="ref-102"></a>XRPL, "server_definitions" method reference.
...
103. <a id="ref-103"></a>XRPL, "Partial Payments" concept documentation.
     https://raw.githubusercontent.com/XRPLF/xrpl-dev-portal/master/docs/concepts/payment-types/partial-payments.md
104. <a id="ref-104"></a>Anza, "Agave CHANGELOG".
     https://raw.githubusercontent.com/anza-xyz/agave/master/CHANGELOG.md
105. <a id="ref-105"></a>Bitcoin Core, "src/rpc/blockchain.cpp".
     https://raw.githubusercontent.com/bitcoin/bitcoin/master/src/rpc/blockchain.cpp
106. <a id="ref-106"></a>Cosmos SDK, "ADR-032: Typed Events" (status: proposed).
     https://raw.githubusercontent.com/cosmos/cosmos-sdk/main/docs/architecture/adr-032-typed-events.md
107. <a id="ref-107"></a>Sui, "GraphQL and Archival Store Complete the Sui Data
     Stack". https://www.sui.io/blog/graphql-archival-store-sui-data-stack
108. <a id="ref-108"></a>Aptos, "aptos-core: api/src/transactions.rs".
     https://raw.githubusercontent.com/aptos-labs/aptos-core/main/api/src/transactions.rs
109. <a id="ref-109"></a>Go Ethereum, "all: remove personal RPC namespace", PR
     30704\. https://github.com/ethereum/go-ethereum/pull/30704
110. <a id="ref-110"></a>Bitcoin Core, "doc/design/multiprocess.md".
     https://raw.githubusercontent.com/bitcoin/bitcoin/master/doc/design/multiprocess.md
111. <a id="ref-111"></a>Ethereum, "JSON-RPC: Add finalized and safe blocks",
     execution-apis PR 200. https://github.com/ethereum/execution-apis/pull/200
112. <a id="ref-112"></a>NEAR, "RPC end point to configure broadcast transaction
     await", nearcore issue 6837. https://github.com/near/nearcore/issues/6837
113. <a id="ref-113"></a>Algorand, "algod: Sunset v1 handlers", go-algorand PR
     4847\. https://github.com/algorand/go-algorand/pull/4847
114. <a id="ref-114"></a>Anza, "Remove support for deprecated rpc endpoints",
     agave PR 1809. https://github.com/anza-xyz/agave/pull/1809
115. <a id="ref-115"></a>logos-execution-zone,
     `lez/indexer/service/protocol/src/lib.rs:444`, commit `47eba25`.
     https://github.com/logos-blockchain/logos-execution-zone
116. <a id="ref-116"></a>Cosmos SDK, "ADR-019: Protocol Buffer State Encoding".
     https://raw.githubusercontent.com/cosmos/cosmos-sdk/main/docs/architecture/adr-019-protobuf-state-encoding.md
117. <a id="ref-117"></a>Coinbase, "mesh-specifications" repository.
     https://raw.githubusercontent.com/coinbase/mesh-specifications/master/README.md
118. <a id="ref-118"></a>logos-blockchain,
...
119. <a id="ref-119"></a>Source read for the per-function gap notes, at commit
     `ecb2cc6` (logos-blockchain) and `47eba25` (logos-execution-zone). Files
     cited include `nodes/node/binary/src/api/openapi.rs`,
     ...
     https://github.com/logos-blockchain/logos-blockchain
120. <a id="ref-120"></a>Appendix: Logos API Surfaces, this repository, which
